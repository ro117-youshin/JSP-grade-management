...
<% 
	...
	int mat=Integer.parseInt(request.getParameter("math")); // 데이터베이스 연결 // 데이터베이스 접근
	Class.forName("com.mysql.cj.jdbc.Driver"); //JDBC 드라이버 로드 
	Connection conn=DriverManager.getConnection("jdbc:mysql://localhost/kopo08","root","root"); 
	// 정정하기 (UPDATE) :
	Statement stmt0=conn.createStatement(); 
	// Statement(sql 쿼리 전달) 변수에 database연결-명령어 저장; 
	String QueryTxt0=String.format("update examtable set name='"+name+"' , kor="+kor+" , eng="+eng+" , mat="+mat+" where studentid="+studentid+" ;"); 
	stmt0.execute(QueryTxt0); 
	
	// sql쿼리 실행 --> 학번 기준으로 데이터 찾아 이름, 국어, 영어, 수학 데이터 수정.
	...
	ResultSet rset2 = stmt2.executeQuery("select count(*) from examtable;"); // sql 쿼리 실행하여 나온 결과(ResultSet) 저장;

	// 수정 결과 조회하기 (전체 데이터)
	Statement stmt3 = conn.createStatement(); // Statement(sql 쿼리 전달) 변수에 database연결-명령어 저장;
	String QueryTxt3 = String.format("select * from examtable order by studentid asc;");
	ResultSet rset3 = stmt3.executeQuery(QueryTxt3);
%>
<% 
	// 데이터베이스 테이블에서 읽어온 데이터 준비 
	int dataPerPage=10; // 한 페이지당 출력 개수 결정; 
	int IndexLength=10; // 목록 번호 출력 개수결정; 
	...
	int revisedDataCnt=0; // ResultSet 순서 카운트 
	int revisedDataNum=0; // 수정한 데이터 번호 

	// 몇 번째 데이터를 수정했는지 계산 
	while (rset3.next()) {
		revisedDataCnt++; 
		if (rset3.getInt(2)==studentid) { 
			break; 
		} 
	} 
	revisedDataNum=revisedDataCnt; // 목록의첫 페이지 번호 계산 (목록 번호 출력용) 
		 
	String dataNum=request.getParameter("from"); 

	if (dataNum==null) { // 접속 시 first_pageNum 결정==> Parameter로 값 받기 전
		first_pageNum = (revisedDataNum/(IndexLength*dataPerPage))*IndexLength+1;
	...
	if (fromPT01 == null) { // 접속 시 첫 번째 출력 데이터 번호 결정 ==> Parameter로 값 받기 전
		fromPT = revisedDataNum - revisedDataNum%dataPerPage + 1;
	} else {
...
<% 
	// ##########출력 시작########## 
	// ##1. 페이지 번호 
	out.println("<h1>성적 조회 후 정정 완료<br>현재 페이지 : "+currentPage+"</h1>");
	...
	while (rset1.next()) { // ResultSet 한 줄씩 읽어가며 --> 공백이 아닌 경우 출력
		...
		if (rset1.getInt(2) == studentid) { // '학번' 기준으로 확인하여 정정한 데이터인 경우 노란색 표시해주기
			out.println("<tr id=revised>"); // 출력 칸 형식 : 가로길이 50, 가운데정렬(align)
			out.println("<td width=50><p align=center>"+rset1.getString(1)+"</p></td>");
			out.println("<td width=50><p align=center>"+rset1.getInt(2)+"</a></p></td>");
			out.println("<td width=50><p align=center>"+rset1.getInt(3)+"</p></td>");
			out.println("<td width=50><p align=center>"+rset1.getInt(4)+"</p></td>");
			out.println("<td width=50><p align=center>"+rset1.getInt(5)+"</p></td>");
	...
	int backward; 
	// 변수 : << 화살표 클릭 시의 목록 첫 번째 번호결정 
	int backward1=first_pageNum - IndexLength; 
	if (backward1 < (1+ IndexLength)) { // 리스트목록 두 번째 시작점보다 작으면, 1 
		backward=1; 
	...
	// 4-1. << 화살표 
	out.println("<td width=50><p align=center><a href='./AllviewDB.jsp?from="+((backward-1)*dataPerPage+1) +"&count="+dataPerPage+"'>"+"<<"+"</a></p></td>");

	// 4-2. 목록 번호
	for (int i = 0; i < IndexLength; i++) { // 목록 번호가 lastPage보다 작거나 같은 경우만 출력 
		if ((first_pageNum+i) <= lastPage) { 
			if ( (first_pageNum+i)==currentPage) {
			out.println("<td width=50 id=current><p align=center><a href='./AllviewDB.jsp?from="+((first_pageNum+i-1)*dataPerPage+1)+"&count="+dataPerPage+"'>"+(first_pageNum+i)+"</a></p></td>");
			} else {
			out.println("<td width=50 id=other<p align=center><a href='./AllviewDB.jsp?from="+((first_pageNum+i-1)*dataPerPage+1)+"&count="+dataPerPage+"'>"+(first_pageNum+i)+"</a></p></td>");
			}
		}
	};
	...
	if (currentPage < totalData/(IndexLength*dataPerPage)*IndexLength+1) {
		out.println("<td width=50><p align=center><a href='./AllviewDB.jsp?from="+((forward-1)*dataPerPage+1)+"&count="+dataPerPage+"'>"+">>"+"</a></p></td>");
	// 더 이상 뒤로 갈 페이지가 없는 경우 --> 마지막 페이지 출력
	} else {
		out.println("<td width=50><p align=center><a href='./AllviewDB.jsp?from="+((lastPage-1)*dataPerPage+1)+"&count="+dataPerPage+"'>"+">>"+"</a></p></td>");
	}
